...
void loop() {
  ...
  String barometer_hpa_topic = deviceName;
  barometer_hpa_topic.concat("/");
  barometer_hpa_topic.concat(String(device_set));
  barometer_hpa_topic.concat("/barometer/hectoPascal");
  char barometer_hpa_topic_buf[barometer_hpa_topic.length()+1];
  barometer_hpa_topic.toCharArray(barometer_hpa_topic_buf, barometer_hpa_topic.length()+1);
  String barometer_inhg_topic = deviceName;
  barometer_inhg_topic.concat("/");
  barometer_inhg_topic.concat(String(device_set));
  barometer_inhg_topic.concat("/barometer/inchHg");
  char barometer_inhg_topic_buf[barometer_inhg_topic.length()+1];
  barometer_inhg_topic.toCharArray(barometer_inhg_topic_buf, barometer_inhg_topic.length()+1);
  
  // Lookup for your altitude
  ...
  if (now - lastMsg > 1000) {
    ...
    if (checkBound(newBaro, baro, diff) || forceMsg) {
      ...
      client.publish(barometer_hpa_topic_buf, String(baro_hpa).c_str(), true);
      client.publish(barometer_inhg_topic_buf, String(baro_inhg).c_str(), true);
      blink_blue();
    }
